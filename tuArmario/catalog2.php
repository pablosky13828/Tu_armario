<?php
	...
	//Info de Producto
	$categoria = isset($_GET['categoryID']) ? $_GET['categoryID'] : '';
	$talla = isset($_GET['talla']) ? $_GET['talla'] : '';
	$novedades = isset($_GET['novedades']) ? $_GET['novedades'] : '';
	...
	if($categoria == ''){
		...
		if($talla == ''){	
			...
			if($novedades == ''){
				...
				$sql = $conex->prepare("SELECT idProduct, carpeta, nameProduct, categoryID, talla, novedades, precio FROM products WHERE novedades=? AND activo=1");
				$sql->execute([$novedades]);
				$resultado = $sql->fetchAll(PDO::FETCH_ASSOC);
			}
		}
		else{
			$sql = $conex->prepare("SELECT idProduct, carpeta, nameProduct, categoryID, talla, novedades, precio FROM products WHERE talla=? AND activo=1");
			$sql->execute([$talla]);
			$resultado = $sql->fetchAll(PDO::FETCH_ASSOC);
		}
	}
	else{
		$sql = $conex->prepare("SELECT idProduct, carpeta, nameProduct, categoryID, talla, novedades, precio FROM products WHERE categoryID=? AND activo=1");
				...
				<table class='catalog-box'>
					...
					<!--Filtros-->
						<td class='catalog-filtros'>
							...
							<div>
								<label> <a class='txt-min red <?php if($categoria==1){echo 'selected';} ?>' href="catalog2.php?categoryID=1"> • Camisetas </input></a> </label><br>	
								<label> <a class='txt-min red <?php if($categoria==2){echo 'selected';} ?>' href="catalog2.php?categoryID=2"> • Sudaderas </input></a> </label><br>
								<label> <a class='txt-min red <?php if($categoria==3){echo 'selected';} ?>' href="catalog2.php?categoryID=3"> • Chaquetas	</input></a> </label><br>
								<label> <a class='txt-min red <?php if($categoria==4){echo 'selected';} ?>' href="catalog2.php?categoryID=4"> • Pantalones</input></a> </label><br>		
								<label> <a class='txt-min red <?php if($categoria==5){echo 'selected';} ?>' href="catalog2.php?categoryID=5"> • Chandals	</input></a> </label><br>							
							</div>
							<br>
							...
							<div>
								<label> <a class='txt-min red <?php if($talla=='S'){echo 'selected';} ?>' href="catalog2.php?talla=S"> • S</a> </label><br>
								<label> <a class='txt-min red <?php if($talla=='M'){echo 'selected';} ?>' href="catalog2.php?talla=M"> • M</a> </label><br>
								<label> <a class='txt-min red <?php if($talla=='L'){echo 'selected';} ?>' href="catalog2.php?talla=L"> • L</a> </label>
							</div>
							<br>
							...
							<div>
								<label> <a class='txt-min red' href="catalog.php"> • Todos</a> </label><br>
								<label> <a class='txt-min red <?php if($novedades==1){echo 'selected';} ?>' href="catalog2.php?novedades=1"> • Novedades</a> </label><br>
							</div>
